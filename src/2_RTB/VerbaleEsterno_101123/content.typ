...
Inoltre, dato che i ruoli all'interno del team ruoteranno ad ogni Sprint, una durata media più estesa dovrebbe contribuire a consolidare le responsabilità connesse ai vari ruoli. La Proponente ha manifestato il suo accordo riguardo a questa modalità di lavoro e si è offerta di organizzare incontri periodici di Stato di Avanzamento Lavori (SAL) alla conclusione di ciascuno Sprint.

A titolo di esempio, considerando che il primo Sprint è iniziato oggi, il prossimo incontro SAL sarà fissato per venerdì 24 novembre, a distanza di esattamente due settimane.
...
- *Implementazione di uno script di simulazione:* Sarà sviluppato uno script in grado di simulare l'attività di uno o più sensori. Lo script emetterà uno stream di dati strutturato, ad esempio in formato JSON, inviandolo a Kafka;

- *Utilizzo di Docker Composed:* Il team adotterà Docker Composed per creare un ambiente containerizzato di base. Questo ambiente conterrà le immagini dei vari componenti dell'architettura, facilitando la gestione e la distribuzione delle risorse;

- *Creazione di uno Sprint Backlog:* Verrà compilato uno Sprint Backlog che conterrà le User Stories rilevanti per la fase iniziale dello sviluppo del _Proof of Concept_ (POC). Questo Sprint Backlog servirà come guida per le attività e le priorità da osservare durante il primo Sprint.

=== Chiarimenti tecnici
La Proponente ha consigliato di sviluppare un singolo script in Python per simulare l'attività di uno o più dispositivi operanti in modalità asincrona. In questa fase iniziale, l'attenzione principale sarà sulla struttura e il funzionamento del meccanismo di simulazione, senza necessariamente prestare particolare attenzione al realismo dei dati in uscita. I dispositivi possono essere definiti all'interno di un file CSV seguendo un formato predefinito che include informazioni anagrafiche come:
